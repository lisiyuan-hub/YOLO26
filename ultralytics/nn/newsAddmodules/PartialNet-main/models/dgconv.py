        ...
        if self.training:
            ...
            if self.pre_epoch == 0 or global_epoch >= self.pre_epoch:
                ...
                if consistent_PConv:
                    ...
                    x1 = F.conv2d(
                        x1, pconv_weight, self.conv.bias, self.conv.stride, self.conv.padding, self.conv.dilation
                    )
                    x_out = torch.cat((x1, x2), 1)
                else:
                    x_out = F.conv2d(
                        x, masked_weight, self.conv.bias, self.conv.stride, self.conv.padding, self.conv.dilation
                    )
