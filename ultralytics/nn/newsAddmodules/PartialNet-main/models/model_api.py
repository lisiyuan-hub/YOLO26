...
class LitModel(pl.LightningModule):
    def __init__(self, num_classes, hparams):
        ...
        if "partialnet" in hparams.model_name:
            ...
            self.model = create_model(hparams.model_name, pretrained=hparams.pretrained, num_classes=num_classes)
    ...
    def _calculate_loss(self, batch, mode="train"):
        ...
        if mode == "train":
            ...
            self.log(f"{mode}_loss", loss, sync_dist=sync_dist)
            self.log(f"{mode}_acc1", acc1, sync_dist=sync_dist)
            self.log(f"{mode}_acc5", acc5, sync_dist=sync_dist)
