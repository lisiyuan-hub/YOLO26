...
class MultiheadDiffAttnCrossV1(nn.Module):
    """Cross attention with MultiheadDiffAttn In this version, the query is from decoder.
    """
...
class MultiheadDiffAttnCrossV2(nn.Module):
    """Cross attention with MultiheadDiffAttn In this version, the query is from encoder + skip connection is added.
    """
...
class FEA(nn.Module):
    ...
    def compute_weighted_edges(self, edges_stack):
        ...
        for idx, (i, j) in enumerate(zip(self.indices[0], self.indices[1])):
            ...
            w_edge += 1.0 / self.m * diff_matrix[i, j, ...]
        return w_edge
...
class DSEBFusion(nn.Module):
    """if use_command is 'no', the block will not be used if use_command is 'dat', the block will only use DiffAttn if
    use_command is 'fea', the block will only use Feature Edge Amplification if use_command is 'dat-fea', the block
    will use both DiffAttn and Feature Edge Amplification if use_command is 'dog', the block will use difference of
    Gaussian (with learnable sigma) for feature sharpening if ues_command is 'dat-dog', the block will use both
    DiffAttn and difference of Gaussian if use_command includes 'seq', the block will use sequential attention (last
    is DiffAttn).
    """
    ...
    def __init__(
        ...
        if self.use_dog:
            self.sigma_raw_1 = nn.Parameter(torch.randn(1, _dim, 1, 1) - 0.4, requires_grad=True)
            self.sigma_raw_2 = nn.Parameter(torch.randn(1, _dim, 1, 1) - 0.0, requires_grad=True)
    ...
    def apply_diffattn(self, x):
        ...
        diff = diff.view(
            diff.shape[0], diff.shape[2], diff.shape[1] // self.input_size, diff.shape[1] // self.input_size
        )
        return diff * x
