...
class DifferentialDualStreamAttention(nn.Module):
    ...
    def __init__(
        self, dim, window_size, num_heads, depth=1, qkv_bias=True, qk_scale=None, attn_drop=0.0, proj_drop=0.0
    ):
        super().__init__()
        self.dim = dim
    ...
    def forward_sa(self, x_concat):
        """Self-Attention: 在 batch 維度 concat，加入跨流交互 x_concat: [2*B, N, C] - torch.cat([x_windows, y_windows], dim=0) 返回:
        [2*B, N, C].
        """
    ...
    def forward_ca(self, x_concat):
        """Cross-Attention: 在 sequence 維度 concat，加入跨流交互 x_concat: [B, 2*N, C] - torch.cat([x_windows, y_windows],
        dim=-2) 返回: [B, 2*N, C].
        """
...
class LFSB(nn.Module):
    def __init__(self, dim, HW, num_heads=8, window_size=4, depth=2, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.input_resolution = (HW, HW)
        self.num_heads = num_heads
        self.window_size = window_size
        ...
        # Differential 注意力
        self.diff_attention = DifferentialDualStreamAttention(dim, to_2tuple(self.window_size), num_heads, depth)

        self.feedforward = DualStreamSeq(
            DualStreamBlock(
                LayerNorm2d(dim), nn.Conv2d(dim, dim * 2, 1), nn.Conv2d(dim * 2, dim * 2, 3, padding=1, groups=dim * 2)
            ),
            DualStreamGate(),
    ...
    def forward(self, DATA):
        ...
        xx_windows, yy_windows = self.diff_attention.forward_sa(torch.cat([x_windows, y_windows], dim=0)).chunk(
            2, dim=0
        )

        # Cross-Attention: sequence 維度 concat
        xy_windows, yx_windows = self.diff_attention.forward_ca(torch.cat([x_windows, y_windows], dim=-2)).chunk(
            2, dim=-2
        )
