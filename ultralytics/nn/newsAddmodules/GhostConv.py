...
from timm.models.layers import CondConv2d, DropPath, SqueezeExcite, drop_path, hard_sigmoid
...
class ConvBnAct(nn.Module):
    ...
    def __init__(
        ...
        self.conv = DynamicConv(
            in_chs, out_chs, kernel_size, stride, dilation=dilation, padding=pad_type, num_experts=num_experts
        )
...
class GhostBottleneck(nn.Module):
    ...
    def __init__(
        self, in_chs, out_chs, dw_kernel_size=3, stride=1, act_layer=nn.ReLU, se_ratio=0.0, drop_path=0.0, num_experts=4
    ):
        super().__init__()
        ...
        self.se = (
            _SE_LAYER(mid_chs, se_ratio=se_ratio, act_layer=act_layer if act_layer is not nn.GELU else nn.ReLU)
            if has_se
            else None
        )
