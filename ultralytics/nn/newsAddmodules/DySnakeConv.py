...
from ultralytics.nn.modules import Conv, Bottleneck, C3k2
from ultralytics.nn.modules.block import PSABlock, C3k
 
__all__ = ['DySnakeConv','DySnakeConvC3k2']
...
class Bottleneck_DySnakeConv(Bottleneck):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), den=None, e=0.5):
        ...
        self.cv1 = DySnakeConv(c1,c_)  # 在这里可以替换一种Conv，也可以都替换。自己可以做一下消融实验
...
class C3k_DySnakeConv(C3k):
    def __init__(self, c1, c2, n=1, den=None, shortcut=False, g=1, e=0.5, k=3):
        ...
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Bottleneck_DySnakeConv(c_, c_, shortcut, g, k=(k, k), den=den, e=1.0) for _ in range(n)))
class DySnakeConvC3k2(C3k2):
    ...
    def __init__(self,c1: int,c2: int,n: int = 1,c3k: bool = False,e: float = 0.5,attn: bool = False,g: int = 1,shortcut: bool = True,):
        ...
        self.m = nn.ModuleList(
            nn.Sequential(
                Bottleneck_DySnakeConv(self.c, self.c, shortcut, g),
                PSABlock(self.c, attn_ratio=0.5, num_heads=max(self.c // 64, 1)),
            )
            if attn
            else C3k_DySnakeConv(self.c, self.c, 2, shortcut, g)
            if c3k
