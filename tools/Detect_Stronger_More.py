...
class DataAugmentationOnDetection:
    ...
    def resize_keep_ratio(self, image, boxes, target_size):
        """参数类型： image：Image.open(path)， boxes:Tensor， target_size:int 功能：将图像缩放到size尺寸，调整相应的boxes,同时保持长宽比（最长的边是target
        size.
        """
        old_size = image.size[0:2]  # 原始图像大小
        # 取最小的缩放比例
        ratio = min(
            float(target_size) / (old_size[i]) for i in range(len(old_size))
...
if __name__ == "__main__":
    # 图像和标签文件夹
    image_path = r"valid\images"
    label_path = r"valid\labels"
    save_path = r"valid\Augumentation"
    # 运行
