...
def split_dataset_refined(src_images_path, src_labels_path, save_path, train_ratio=0.6, val_ratio=0.2, test_ratio=0.2):
    ...
    for split_name, samples in dataset_splits.items():
        ...
        for img_name in samples:
            ...
            # 拷贝图片
            shutil.copy(
                os.path.join(src_images_path, img_name), os.path.join(save_path, split_name, "images", img_name)
            )
            # 拷贝标签
            shutil.copy(
                os.path.join(src_labels_path, lbl_name), os.path.join(save_path, split_name, "labels", lbl_name)
            )
